(function() {
    ...
    HTMLWidgets.widget({
        ...
        renderValue: function(el, data, instance) {
            ...
            if (server && HTMLWidgets.shinyMode && typeof options.ajax === 'object' &&
                ...
                options.ajax.dataSrc = function(json) {
                    ...
                    var table = $table.DataTable(),
                        order = table.colReorder.order(),
                        flag = true,
                        i, j, row;
                    for (i = 0; i < order.length; ++i)
                        if (order[i] !== i) flag = false;
                    if (flag) return data;
            ...
            // check the options.
            var colReorderEnabled = function() {
                return "colReorder" in options;
            };
            ...
            if (!crosstalkOptions.group) {
                ...
                applyCrosstalkFilter({
                    value: instance.ctfilterHandle.filteredKeys
                });
                ...
                applyCrosstalkSelection({
                    value: instance.ctselectHandle.value
                });
            }
                ...
                filterRow.each(function(i, td) {
                    ...
                    if (inArray(type, ['factor', 'logical'])) {
                        $input.on({
                            ...
                            input: function() {
                                var v1 = JSON.stringify(filter[0].selectize.getValue()),
                                    v2 = $input.val();
                                if (v1 === '[]') v1 = '';
                        ...
                        r1 = Math.round(r1 * scale);
                        r2 = Math.round(r2 * scale);
                    ...
                    var customFilter = function(settings, data, dataIndex) {
                        ...
                        if (type === 'number' || type === 'integer') {
                            ...
                            v = new Date(v);
                            r0 = new Date(r[0] / scale);
                            r1 = new Date(r[1] / scale);
                            if (v >= r0 && v <= r1) return true;
                        } else if (type === 'factor') {
            ...
            if (data.editable) table.on('dblclick.dt', 'tbody td', function(e) {
                ...
                for (var i = 0; i < target.length; i++) {
                    (function(cell, current) {
                        var $cell = $(cell),
                            html = $cell.html();
                        var _cell = table.cell(cell),
                            value = _cell.data(),
                            index = _cell.index().column;
                        var $input;
                        ...
                        if (immediate) $input.on('blur', function(e) {
                            ...
                            if (valueNew !== value) {
                                ...
                                if (HTMLWidgets.shinyMode) {
                                    changeInput('cell_edit', [cellInfo(cell)], 'DT.cellInfo', null, {
                                        priority: 'event'
                                    });
                                }
            ...
            var selMode = data.selection.mode,
                selTarget = data.selection.target;
            ...
            if (inArray(selMode, ['single', 'multiple'])) {
                ...
                var initSel = function(x) {
                    if (x === null || typeof x === 'boolean' || selTarget === 'cell') {
                        ...
                        return {
                            rows: $.makeArray(x.rows),
                            cols: $.makeArray(x.cols)
                        };
                    }
                }
                var selected = data.selection.selected;
                var selected1 = initSel(selected).rows,
                    selected2 = initSel(selected).cols;
                ...
                var selectable1 = initSel(selectable).rows,
                    selectable2 = initSel(selectable).cols;
                ...
                var shiftSelRowsIndex = function(start, end) {
                    var indexes = server ? DT_rows_all : table.rows({
                        search: 'applied'
                    }).indexes().toArray();
                    start = indexes.indexOf(start);
                    end = indexes.indexOf(end);
                ...
                if (inArray(selTarget, ['row', 'row+column'])) {
                    ...
                    var onlyKeepSelectableRows = function() {
                        ...
                        if (nonselectable) {
                            ...
                            selected1 = $(selected1).not(selectable1.map(function(i) {
                                return -i;
                            })).get();
                        } else {
                    ...
                    var selectRows = function(ignoreSelectable) {
                        ...
                        if (server) {
                            ...
                            var selected0 = selected1.map(function(i) {
                                return i - 1;
                            });
                    ...
                    table.on('mousedown.dt', 'tbody tr', function(e) {
                        ...
                        changeInput('row_last_clicked', serverRowIndex(thisRow.index()), null, null, {
                            priority: 'event'
                        });
                    ...
                    changeInput('rows_selected', selected1);
                    if (server) table.on('draw.dt', function(e) {
                        selectRows(false);
                    });
                ...
                if (inArray(selTarget, ['column', 'row+column'])) {
                    ...
                    var onlyKeepSelectableCols = function() {
                        ...
                        if (nonselectable) {
                            ...
                            selected2 = $(selected2).not(selectable2.map(function(i) {
                                return -i;
                            })).get();
                        } else {
                ...
                if (selTarget === 'cell') {
                    ...
                    var onlyKeepSelectableCells = function() {
                        ...
                        if (nonselectable) {
                            selected3.map(function(ij) {
                                ...
                                if (findIndex([-ij[0], -ij[1]], selectable3) === -1) {
                                    out.push(ij);
                                }
                            });
                        } else {
                            selected3.map(function(ij) {
                                if (findIndex(ij, selectable3) > -1) {
                                    out.push(ij);
                                }
                            });
                        }
                    ...
                    table.on('click.dt', 'tbody td', function() {
                        var $this = $(this),
                            info = tweakCellIndex(table.cell(this));
                        if ($this.hasClass(selClass)) {
                            $this.removeClass(selClass);
                            ...
                            if (selMode === 'single') $(table.cells().nodes()).removeClass(selClass);
                            $this.addClass(selClass);
                            selected3 = selMode === 'single' ? [
                                    [info.row, info.col]
                                ] :
                                unique(selected3.concat([
                                    [info.row, info.col]
                                ]));
                        }
            ...
            table.on('click.dt', 'tbody td', function() {
                changeInput('cell_clicked', cellInfo(this), null, null, {
                    priority: 'event'
                });
            })
            ...
            methods.addRow = function(data, rowname, resetPaging) {
                var n = table.columns().indexes().length,
                    d = n - data.length;
                if (d === 1) {
    ...
    Shiny.addCustomMessageHandler('datatable-calls', function(data) {
        ...
        var methods = table.shinyMethods,
            call = data.call;
